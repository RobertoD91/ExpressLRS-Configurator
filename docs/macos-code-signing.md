...
| --- | --- |
| Created by the **Account Holder** (Admin access) | **Yes** |
| "Admin" key created by any other admin | No, Apple answers 403 |
| "App Manager" key | No, it cannot manage certificates at all (403 already when listing them) |
| Any key | Fine for **notarization**, App Manager access is enough there |

On an individual developer account you are the Account Holder, so an Admin
key you generated yourself qualifies. Since June 2023 the
[App Store Connect API](https://developer.apple.com/documentation/appstoreconnectapi/certificates)
accepts such keys for creating Developer ID certificates.
